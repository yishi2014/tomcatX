...
public class TableMeta {
    ...
    private String isTree;    //表格内容是否使用树展示Y/N
    ...
    private String treeConfigEnable; //是否开启树配置 {是：1}|{否：0}
    ...
    public TableMeta(){}
    public TableMeta(DatabaseMetaData dbmd,String tableName){
        this(dbmd,null,null,tableName,null);
    }
    public TableMeta(DatabaseMetaData dbmd,String catalog,String tableSchem,String tableName,String columnNamePattern )  {
        if(catalog!=null){
        ...
        if(columnNamePattern!=null){
            ...
            if(columnNamePattern.trim().equals("")){
                columnNamePattern=null;
            }
        }
        this.entityPackage=DEFAULT_ENTITY_PACKAGE;
        this.controllerPackage=DEFAULT_CONTROLLER_PACKAGE;
        this.servicePackage=DEFAULT_SERVICE_PACKAGE;
        this.serviceImplPackage=DEFAULT_SERVICE_IMPL_PACKAGE;
        this.javaPathPrefix=DEFAULT_JAVA_DIR_PREFIX;
        this.htmlPathPrefix=DEFAULT_HTML_DIR_PREFIX;
        this.entityName= RegexUtil.toCamel(tableName);
        this.controllerName=RegexUtil.toCamel(tableName+"_controller");
        this.serviceName=RegexUtil.toCamel(tableName+"_service");
        this.serviceImplName=RegexUtil.toCamel(tableName+"_service_impl");
        this.select=true;
        this.update=true;
        this.insert=true;
        this.delete=true;
        this.excel=true;
        this.encode="utf-8";
        this.htmlName=RegexUtil.toCamel1Low(tableName+".html");
        ...
        try(
                //表中各个字段的信息
                ResultSet rs = dbmd.getColumns(catalog, tableSchem, tableName, columnNamePattern);

        ){
            this.columnMetas=new LinkedHashMap<>();
            ...
            while (rs.next()){
                ...
                columnMeta.setJavaType(JDBCTypeParser.getJavaType(dbmd.getDatabaseProductName().toUpperCase(),columnMeta));
        ...
        if(i==1){
            try(ResultSet pkrs=dbmd.getPrimaryKeys(null,tableSchem,tableName);){
                while(pkrs.next()){
                    String columnName=pkrs.getString("COLUMN_NAME").toUpperCase();
                    this.getColumnMetas().get(columnName).setPrimaryKey(true);
                    this.pks.add(columnName);
                }
            }catch (Exception e){
                e.printStackTrace();
            }
...
//        }
    ...
    public TableMeta(String tableSchem, String tableName, String remarks, String tableType) {
