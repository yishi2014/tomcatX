...
public class StringBitwiseOperation {
    ...
    public boolean compare(char cur, char peek) {// 如果是peek优先级高于cur，返回true，默认都是peek优先级要低
        ...
        if (operatPriority.get(peek) >= operatPriority.get(cur)) {
            result = true;
        }
        return result;
    }
...
//        return result;
//    }
    private String notOperatoin(String firstValue){
        if(firstValue.indexOf("!")>-1)
        return String.valueOf(new Integer(firstValue.substring(1))>0?0:1);
        return firstValue;
    }
    private String calculate(String firstValue, String secondValue, char currentOp) {
        String result = "";
        switch (currentOp) {
            case '&':
                result = String.valueOf(new Integer(firstValue)&new Integer(secondValue));
                break;
            case '|':
                result = String.valueOf(new Integer(firstValue)|new Integer(secondValue));
                break;
    ...
    public Integer calculate(String expression) {
        ...
        while (!postfixStack.isEmpty()) {
            ...
            if (!isOperator(currentValue.charAt(0))) {// 如果不是运算符则存入操作数栈中
                ...
                // 将负数标记符改为负号
                firstValue =notOperatoin(firstValue);
                secondValue = notOperatoin(secondValue);
    ...
    public  int conversion(String expression) {
        int result = 0;
        StringBitwiseOperation cal = new StringBitwiseOperation();
        try {
    ...
    public static void main(String[] args) {
        ...
        int result = new StringBitwiseOperation().conversion(expression);
